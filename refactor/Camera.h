...
public:
    ...
    double* calcSphereIntersect(double* ray, double* pixpt, double depth, double lastPos);
    double* throwRay(int x, int y);

    double* calcSphereColor(int position, double* intersect, double* pixpt, double depth, double* ray, double lastPos);
    double* calcFaceColor(int position);
    double* diffuseReflection(double* normal, double* intersect);
    ...
    inline void setDiffuse(double x, double y, double z) {diffuse[0] = x; diffuse[1] = y; diffuse[2] = z;}
    inline void setSpecular(double x, double y, double z){specular[0] = x; specular[1] = y; specular[2] = z;}
