...
void Scene::write(string filename) {
    ...
    for(unsigned int i = 0; i < colors.size(); i++) {
        color = colors[i];
        outfile << (int)(color[0] * 255) << " " << (int)(color[1] * 255) << " " << (int)(color[2] * 255) << " ";
    }
}
    ...
    while(!file.eof()) {
        ...
        if(token != nullptr){
            if (strcmp(token, "ambient") == 0) {
                ...
                for(int i = 0; i < 3; i++) {
                    token = strtok(NULL, " ");
                    diffuse[i] = stod(token);
                }
                cam.setDiffuse(diffuse[0], diffuse[1], diffuse[2]);
            }
            else if (strcmp(token, "specular") == 0) {
                double specular[3];
                for(int i = 0; i < 3; i++) {
                    token = strtok(NULL, " ");
                    specular[i] = stod(token);
                }
                cam.setSpecular(specular[0], specular[1], specular[2]);
            }            
